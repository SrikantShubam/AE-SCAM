...
internal object GuardianTelemetry {
    ...
    private val notificationCategories = setOf(
        "com_google_android_apps_messaging",
        "com_whatsapp",
        "com_whatsapp_w4b",
        "com_samsung_android_messaging",
        "com_android_mms",
    )
    internal var testEventSink: ((String, Map<String, String>) -> Unit)? = null
    ...
    fun logScamNotificationPosted(category: String, language: String) {
        val safeCategory = normalizeEnum(
            normalizeToken(category, maxLength = 64),
            notificationCategories,
            "com_android_mms",
        )
        val safeLanguage = normalizeToken(language, maxLength = 16)
        logEvent(
            eventName = "scam_notification_posted",
            fields = mapOf(
                "category" to safeCategory,
                "language" to safeLanguage,
            ),
        )
    }
