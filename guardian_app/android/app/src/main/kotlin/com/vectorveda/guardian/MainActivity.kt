...
class MainActivity : FlutterActivity() {
    companion object {
        ...
        private const val MEDICATION_ALARM_ACK_PREFS = "medication_alarm_ack"
        private const val MEDICATION_ALARM_ACK_QUEUE_KEY = "pending_ack_queue_json"
        ...
        MethodChannel(
            ...
            when (call.method) {
                ...
                "consumePendingAlarmAcknowledgements" -> {
                    result.success(consumePendingMedicationAlarmAcknowledgements())
                }
    ...
    private fun consumePendingMedicationAlarmAcknowledgements(): List<String> {
        val prefs = getSharedPreferences(MEDICATION_ALARM_ACK_PREFS, Context.MODE_PRIVATE)
        val queued = prefs.getString(MEDICATION_ALARM_ACK_QUEUE_KEY, null)
        val payloads = ScamNotificationPayloadQueueCodec.consumeAll(queued)
        if (queued != null) {
            prefs.edit().remove(MEDICATION_ALARM_ACK_QUEUE_KEY).apply()
        }
        return payloads
    }
